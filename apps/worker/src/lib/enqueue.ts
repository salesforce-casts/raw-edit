...
export async function enqueueJob(input: {
  ...
  const [existing] = await db
    ...
    .limit(1);
  if (existing && (existing.status === "SUCCEEDED" || existing.status === "COMPLETED")) {
    return existing;
  }
  ...
  const activeForUser = await db
    ...
    .where(
      and(
        ...
        inArray(processingJobs.status, ["QUEUED", "RUNNING", "ACTIVE"]),
      ),
    );
  const delayMs =
    Number(activeForUser[0]?.count ?? 0) >= config.maxConcurrentJobsPerUser ? retryDelayMs(0) : 0;
  ...
  const bullmqJobId = await createBullmqQueue().enqueue(queueNameForJob(input.type), payload, {
    jobId: key,
    delayMs,
  });
...
export function missingVideo(): never {
  throw new AppError("NOT_FOUND", "Video missing", 404, true);
}
